...
除另有明确标注的第三方材料外，本项目作者原创的源代码以 GNU Affero General Public License v3.0 only（`AGPL-3.0-only`）授权，完整文本见仓库根目录 [`LICENSE`](LICENSE)。该授权主要覆盖 `langGraph/` 中的项目代码和 `download_models.py`，不改变以下第三方源码、依赖、模型、权重或数据的许可证。

项目的个人学术研究和非商业分享定位不是对 AGPL-3.0 代码附加的许可限制。任何使用者仍须同时遵守实际使用到的所有第三方组件条款。
...
Ultralytics 当前官方许可说明明确表示，使用其训练代码产生的模型默认适用 AGPL-3.0；因此本仓库的 `models/yolo/yolo_best.pt` 按该开源路线处理，仓库根目录 `LICENSE` 同时提供了完整 AGPL-3.0 文本。若权重训练或使用基于另行购买的企业许可，则应以对应合同为准。

自训练权重还可能受到训练数据集许可、标注来源、个人信息和其他权利的影响。AGPL-3.0 不会自动授予训练数据中原本不存在的权利；权重提供者仍应确保自己有权训练和分发该模型，并单独说明训练数据许可。
